...
| Rechner (Wegzug-Szenario) | Heutige Steuerleistung der wegziehenden Gruppe: Vermögenssteuer (Ø 0,28 % eff. auf steuerbares Vermögen) und Einkommenssteuer auf Kapitalerträge (~0,5 % des steuerbaren Vermögens). Pauschalbesteuerte bereits im Pareto-Tail der Bins eingerechnet. | NZZ (`nzz_vermoegenssteuer`): Ø Satz 0,28 %; Martínez/KOF (`reichensteuer_studie_ch`): Einkommenssteuer auf Kapital ~0,5 % | Konstanten `VST_RATE`, `EST_RATE` in `useCalculator.js` | Schätzung, keine separate Datendatei |
