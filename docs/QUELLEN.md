...
| Zucman-Steuer | Vorschlag (2 % ab 100 Mio.), heutige Last, Mehraufkommen, Tage bis Mehrbelastung verdient, Medianhaushalt (Jahre bis Steuern aus Vermögenseinkommen verdient), Einordnungen | Zucman (`woz_zucman`, `zucman_g20`); Bilanz (`bilanz300`); NZZ (`nzz_vermoegenssteuer`); Oxfam/TJN/Momentum (`reichensteuer_studie`); Medianhaushalt: BFS HABE (`bfs_habe`), UBS (`ubs`); Einordnungen: BFS (`bfs`), EFV (`efv`), BAG (`bag`) | — (Texte + `sources.json`); Einordnungen `spend_reference.json` | — (Texte); `04` (Bezugsgrössen, §5) |
