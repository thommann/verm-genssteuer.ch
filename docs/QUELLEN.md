...
| Zucman-Steuer | Vorschlag (2 % ab 100 Mio.), heutige Last, Mehraufkommen, Tage bis ganze 2 %-Steuer passiv verdient, normaler Haushalt (Jahre bis ganze Steuer aus Vermögenseinkommen verdient, einkommensbasiert), Einordnungen | Zucman (`woz_zucman`, `zucman_g20`); Bilanz (`bilanz300`); NZZ (`nzz_vermoegenssteuer`); Oxfam/TJN/Momentum (`reichensteuer_studie`); Haushalt: BFS HABE (`bfs_habe`); Einordnungen: BFS (`bfs`), EFV (`efv`), BAG (`bag`) | — (Texte + `sources.json`); Einordnungen `spend_reference.json` | — (Texte); `04` (Bezugsgrössen, §5) |
