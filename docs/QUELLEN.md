...
| Zucman-Steuer | Vorschlag (2 % ab 100 Mio.), heutige Last, Mehraufkommen, Tage bis ganze 2 %-Steuer passiv verdient, normaler Haushalt (praktisch kein passives Einkommen, Steuer nicht passiv aufbringbar), Einordnungen | Zucman (`woz_zucman`, `zucman_g20`); Bilanz (`bilanz300`); NZZ (`nzz_vermoegenssteuer`); Oxfam/TJN/Momentum (`reichensteuer_studie`); Haushalt: ESTV (`estv_vermoegen`), BFS HABE (`bfs_habe`); Einordnungen: BFS (`bfs`), EFV (`efv`), BAG (`bag`) | — (Texte + `sources.json`); Einordnungen `spend_reference.json` | — (Texte); `04` (Bezugsgrössen, §5) |
